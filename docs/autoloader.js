...
function loadMinecraftFont(fontName, baseUrl) {const formats = [{ ext: ".woff2", format: "woff2" },{ ext: ".woff", format: "woff" },{ ext: ".ttf", format: "truetype" }];(function tryNextFormat(index = 0) {if (index >= formats.length) return console.error("Keine Minecraft-Font-Datei konnte geladen werden.")const { ext, format } = formats[index];const fontUrl = baseUrl + ext;const font = new FontFace(fontName, `url(${fontUrl}) format('${format}')`, { style: "normal", weight: "normal", display: "swap" });font.load().then(loadedFont => {document.fonts.add(loadedFont);document.body.style.fontFamily = `'${fontName}', sans-serif`;console.log(`Minecraft-Font (${format}) erfolgreich geladen.`);}).catch(() => {console.warn(`Fehler beim Laden von: ${fontUrl}, versuche nächstes Format...`);tryNextFormat(index + 1);});})();}loadMinecraftFont("Minecraft", "https://cdn.jsdelivr.net/gh/Redminer9630/Website@v1.1.1/docs/minecraft_font");
